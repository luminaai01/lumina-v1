...
from lumina.actions import ActionExecutor, AsyncActionExecutor, AsyncIPythonInterpreter, IPythonInteractive
...
from lumina.prompts.parsers import InterpreterParser, MixedToolParser, PluginParser, ToolStatusCode
...
class AgentForlumina(Agent):
    ...
    def __init__(
        ...
        aggregator: Dict = dict(type=luminaToolAggregator),
        action_hooks: List = [dict(type=luminaActionProcessor)],
...
class MathCoder(AgentForlumina):
...
class AsyncAgentForlumina(AsyncAgent):
...
class AsyncMathCoder(AsyncAgentForlumina):
